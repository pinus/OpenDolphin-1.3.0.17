...
public class RubberBand {
    ...
    public ShapeHolderBase getHolder() {
        return holder;
    }
    ...
    public Pos getPos(double x, double y) {
        if (isLine) {
            if (SchemaUtils.isNear(x, y, holder.getPathX(0), holder.getPathY(0))) {
                return Pos.xSyS;
            }
            if (SchemaUtils.isNear(x, y, holder.getPathX(1), holder.getPathY(1))) {
                return Pos.xEyE;
            }
        }
        if (SchemaUtils.isNear(x, y, holder.getStartX(), holder.getStartY())) {
            return Pos.xSyS;
        }
        if (SchemaUtils.isNear(x, y, holder.getEndX(), holder.getEndY())) {
            return Pos.xEyE;
        }
        if (SchemaUtils.isNear(x, y, holder.getStartX(), holder.getEndY())) {
            return Pos.xSyE;
        }
        if (SchemaUtils.isNear(x, y, holder.getEndX(), holder.getStartY())) {
            return Pos.xEyS;
        }
        return null;
    }
